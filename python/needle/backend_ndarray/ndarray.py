...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        # Count -1s and infer the dimension if needed
        neg_ones = 0
        ...
        if neg_ones > 1:
            ...
            if known_size == 0:
                raise ValueError("Cannot infer dimension when other dimensions are 0")
        ...
        if prod(self.shape) != prod(new_shape):
            ...
            return ValueError("Array is not compact")
        else: 
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        """
        You should call NDArray.make with the new strides and offset, 
        and then immediately .compact() this array.
        """
        shape = self._shape
        ...
        for ax in axes: 
            new_offset += (shape[ax] - 1) * strides[ax]
            new_strides[ax] = -strides[ax]

        flipped = NDArray.make(shape=shape, strides=tuple(new_strides), device=self.device, handle=self._handle, offset=new_offset)
