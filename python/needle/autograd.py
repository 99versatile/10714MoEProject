...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        out_grads = node_to_output_grads_list.get(node, [])
        if len(out_grads) == 0:  # skip nodes that receive no gradient during this backward pass 
            continue

        # Sum up partial adjoints
        total_out_grad = sum_node_list(out_grads)
        node.grad = total_out_grad  # store gradients (adjoints) in grad field

        if node.op is None:
            continue

        in_grads = node.op.gradient_as_tuple(total_out_grad, node) # calculate partial adjoints
        for i, in_grad in zip(node.inputs, in_grads):
            if i.requires_grad:
                node_to_output_grads_list.setdefault(i, []).append(in_grad) # propagate(=accumulate) partial adjoint (=per-edge contribution) for each input
    ### END YOUR SOLUTION
