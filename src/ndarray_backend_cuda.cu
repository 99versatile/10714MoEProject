...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    ...
    // convert gid to multi-dim indices
    int32_t multi_dim_idx[MAX_VEC_SIZE]; 
    size_t gid_temp = gid;
    for (int i=num_dim - 1; i >= 0; i--) {
      multi_dim_idx[i] = gid_temp % shape.data[i];
      gid_temp = gid_temp / shape.data[i];
    }

    // input idx
    size_t idx = offset;
    for (size_t j=0; j < num_dim; j++) {
      idx += multi_dim_idx[j] * strides.data[j];
    } 
    // copy a to out
    out[gid] = a[idx];
  }
...
__global__ void MatmulKernel(const float* a, const float* b, float* out, size_t M, size_t N, size_t P) {
  ...
  for (int t = 0; t < (N + TILE - 1) / TILE; t++) {
    __syncthreads();
    // Cooperatinve loading into shared memory
    if (row < M && t * TILE + tx < N)
      sA[ty][tx] = a[row * N + t * TILE + tx];
    else
      sA[ty][tx] = 0.0f;
    if (col < P && t * TILE + ty < N)
      sB[ty][tx] = b[(t * TILE + ty) * P + col];
    else
      sB[ty][tx] = 0.0f;
    __syncthreads();
